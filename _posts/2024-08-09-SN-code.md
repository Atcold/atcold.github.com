...
Eventually, code is was is run to produce a result.
...
The analytical derivative is $f'(s) = 1.58\cdot2/3 \, (1 - \tanh(2/3\, s)) + 0.05$.
...
> Then, we use gradient descent to find the minimum of $g$, starting for example at $s=0$.
...
and `basic-iteration` is where the standard learning pipeline take place:
